...
cam_url = ''
cam_number = 0
knn_filename = 'maint_knn_model.clf'
...
def config_start(filename):
    ...
    knn_filename = parser.get('KNN', 'filename')
...
def display(faces, process, size, mq, img_frame):
    if process is None:
        logger.error('Error')
    else:
...
if __name__ == "__main__":
    ...
    logger.info("knn_filename {}".format(knn_filename))
        ...
        if flag_cam is not True:
            ...
            if flag_image:
                ...
                if flag_proc:
                    ...
                    exit(1)
        flag = load_face_db(knn_filename)
        if flag is not True:
        ...
        while True:
            ...
            if flag_image:
                if (frame_number % period) == 0:
                    ...
                    detected_faces = recognize_faces(locations, recognitions, knn_model, distance_threshold)
                display(detected_faces, proc, resize, client, frame)
