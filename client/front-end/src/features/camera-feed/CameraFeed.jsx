...
class CameraFeed extends React.Component {
  ...
  render() {
    ...
    const height = SETTINGS.CAMERA_FEED_HEIGHT;

    const imgStyle = { "maxWidth": `${ width }px`, "height": `${ height }px` };

    const stripStyle = this.props.profileData && !this.state.unknown ? ((this.props.profileData.clearanceType === "B") ? "top-strip-access" : "top-strip-denied") : "top-strip";

    const fontAwesomeName = this.props.profileData && !this.state.unknown ?((this.props.profileData.clearanceType === "B") ? "check" : "times-circle") : "check";
      ...
      <div className="camera-feed" >
        ...
        <div className={ alertState }>
          <div className={stripStyle}>
            <FontAwesome className="check" name={fontAwesomeName} size="4x" />
          </div>
